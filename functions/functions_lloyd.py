...
class LloydRelaxation:
    ...
    def build_voronoi(self):
        ...
        self.voronoi = sc.spatial.Voronoi(self.points, qhull_options='Qbb Qc Qx')
    ...
    def relax(self):
        ...
        """
        centroids = []
        for idx in self.voronoi.point_region:  # the region is a series of indices into self.voronoi.vertices
            region = [i for i in self.voronoi.regions[idx] if
                      i != -1]  # remove point at infinity, designated by index -1
            region = region + [region[0]]  # enclose the polygon
            verts = self.voronoi.vertices[region]  # get the vertices for this region
